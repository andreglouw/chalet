 ...
 */
class FlowSensorProperties {
  public:
	FlowSensorProperties(float_t capacity, float_t kFactor);

	void archiveToFile(char *filepath);
    void initializeFromFile(char *filepath);
	String toString();
	void fromString(String str);

	float_t capacity;      //!< capacity, upper limit of flow rate (in l/min)
	float_t kFactor;       //!< "k-factor" (in (pulses/s) / (l/min)), e.g.: 1 pulse/s = kFactor * l/min
	float_t mFactor[10];   //!< multiplicative correction factor near unity, "meter factor" (per decile of flow)
};
...
class FlowMeter {
  public:
    ...
    float_t getCurrentFlowrate();                  //!< Returns the current flow rate since last reset (in l/min).
    float_t getCurrentVolume();                    //!< Returns the current volume since last reset (in l).

    float_t getTotalFlowrate();                    //!< Returns the (linear) average flow rate in this flow meter instance (in l/min).
    float_t getTotalVolume();                      //!< Returns the total volume flown trough this flow meter instance (in l).
    ...
    float_t getCurrentFrequency();                 //!< Returns the pulse rate in the current tick (in 1/s).
    float_t getCurrentError();                     //!< Returns the error resulting from the current measurement (in %).

    unsigned long getTotalDuration();             //!< Returns the total run time of this flow meter instance (in ms).
    float_t getTotalError();                       //!< Returns the (linear) average error of this flow meter instance (in %).

    FlowSensorProperties getProperties() { return this->_properties; }
    void setProperties(FlowSensorProperties properties) {
  ...
  protected:
	...
	elapsedMillis _lastTick;
    unsigned long _currentDuration;               //!< current tick duration (convenience, in ms)
    float_t _currentFrequency;                     //!< current pulses per second (convenience, in 1/s)
    float_t _currentFlowrate = 0.0f;               //!< current flow rate (in l/tick), e.g.: 1 l / min = 1 pulse / s / (pulses / s / l / min)
    float_t _currentVolume = 0.0f;                 //!< current volume (in l), e.g.: 1 l = 1 (l / min) / (60 * s)
    float_t _currentCorrection;                    //!< currently applied correction factor

    unsigned long _totalDuration = 0.0f;          //!< total measured duration since begin of measurement (in ms)
    float_t _totalVolume = 0.0f;                   //!< total volume since begin of measurement (in l)
    float_t _totalCorrection = 0.0f;               //!< accumulated correction factors
...
class FlowSensorCalibration {
  public:
    ...
    FlowSensorCalibration* setCapacity(float_t capacity) {
        this->_properties.capacity = capacity;
        return this;
    }

    FlowSensorCalibration* setKFactor(float_t kFactor) {
        this->_properties.kFactor = kFactor;
        return this;
    }

    FlowSensorCalibration* setMeterFactorPerDecile(unsigned int decile, float_t mFactor) {
    ...
    float_t getMeterFactorPerDecile(unsigned int decile) {
